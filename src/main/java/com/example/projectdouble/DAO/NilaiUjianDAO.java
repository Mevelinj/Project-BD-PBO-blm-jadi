...
public class NilaiUjianDAO {
    ...
    public boolean addNilaiUjian(NilaiUjian nilaiUjian) {
        String sql = "INSERT INTO nilai_ujian (jenis_ujian, nilai, id_mapel, nis) VALUES (?, ?, ?, ?)";
    ...
    public boolean updateNilaiUjian(NilaiUjian nilaiUjian) {
        String sql = "UPDATE nilai_ujian SET jenis_ujian = ?, nilai = ?, id_mapel = ?, nis = ? WHERE id_nilai_ujian = ?";
        try (Connection conn = DBConnect.getConnection();
            ...
            stmt.setInt(3, nilaiUjian.getIdMapel());
            stmt.setString(4, nilaiUjian.getNis());
            stmt.setInt(5, nilaiUjian.getIdNilaiUjian());
            int rowsAffected = stmt.executeUpdate();
    ...
    public List<NilaiUjian> getNilaiUjianByGuru(String nipGuru) {
        ...
        String sql = "SELECT nu.id_nilai_ujian, nu.jenis_ujian, nu.nilai,   " +
                ...
                "JOIN siswa s ON nu.nis = s.nis " +
                "WHERE g.nip = ? " +
                "GROUP BY nu.id_nilai_ujian, nu.jenis_ujian, nu.nilai,   nu.id_mapel, mp.nama_mapel, nu.nis, s.nama " +
                "ORDER BY s.nama,   mp.nama_mapel, nu.jenis_ujian";
        try (Connection conn = DBConnect.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
